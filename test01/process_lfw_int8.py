...
def main():
    ...
    model_path = os.path.join(script_dir, 'yunet_int8.onnx')
    output_dir = os.path.join(script_dir, 'lfw_outputs')
    
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
        
    txt_path = os.path.join(output_dir, 'lfw_detection_results_int8.txt')
    ...
    print(f"Selected 5 LFW images for INT8: {[os.path.basename(p[1]) for p in image_paths]}")
    
    with open(txt_path, 'w', encoding='utf-8') as f_out:
        f_out.write("==================================================\n")
        f_out.write("LFW Face Detection Results using YuNet INT8\n")
        f_out.write("==================================================\n\n")
        
        for idx, (person, img_path) in enumerate(image_paths):
            img_name = os.path.basename(img_path)
            f_out.write(f"[{idx+1}/5] Image: {img_name} (Person: {person})\n")
            print(f"Processing {img_name} (INT8)...")
            ...
            try:
                ...
                if retval and faces is not None:
                    ...
                    for i, face in enumerate(faces):
                        ...
                        # 바운딩 박스(빨간색) 및 랜드마크(초록색) 그리기 (INT8은 다르게 시각화)
            ...
            out_img_path = os.path.join(output_dir, f"result_int8_{img_name}")
            cv2.imwrite(out_img_path, img)
            f_out.write(f"  Saved visualized output to: result_int8_{img_name}\n\n")
